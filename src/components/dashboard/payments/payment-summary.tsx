    ...
    <div className="flex flex-row gap-4 mb-8">
      <div className="flex-1 rounded-lg border-2 border-primary-500/10 bg-white px-12.5 py-8">
        <div className="grid grid-cols-2 gap-6">
          <PayoutSummaryItem
            title={summaryData.previousPayout.title}
            date={summaryData.previousPayout.date}
            amount={summaryData.previousPayout.amount}
            status={summaryData.previousPayout.status}
            className="border-r border-primary-500/10"
          />

          <PayoutSummaryItem
            title={summaryData.nextPayout.title}
            date={summaryData.nextPayout.date}
            amount={summaryData.nextPayout.amount}
            status={summaryData.nextPayout.status}
            className="pl-17"
          />
        </div>
      </div>
...
interface PayoutSummaryItemProps {
  title: string;
  date: string;
  amount: number;
  status: "Paid" | "Pending";
